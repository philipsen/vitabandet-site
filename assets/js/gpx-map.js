(function (global) {
  ...
  // Capture Leaflet in this IIFE. Blowfish's deferred search bundle later
  // declares a top-level `const L` (Fuse.js), which shadows window.L.
  const Leaflet = global['L'] && global['L'].Map ? global['L'] : global.leaflet;
  ...
  class GPXMapHandler {
    ...
    async init() {
      if (!Leaflet || typeof Leaflet.Map !== 'function') { setTimeout(() => this.init(), 50); return; }
